    ...
    private Node balance(Node node) {
        ...
        if (balanceFactor > 1) {
            if (height(node.left.left) >= height(node.left.right)) {
                System.out.println("Performing LL Rotation" +" "+ node.data);
                return rotateRight(node);
            } else {
                System.out.println("Performing LR Rotation" + " " +node.data);
        ...
        if (balanceFactor < -1) {
            if (height(node.right.right) >= height(node.right.left)) {
                System.out.println("Performing RR Rotation" +" "+ node.data);
                return rotateLeft(node);
            } else {
                System.out.println("Performing RL Rotation"+" "+ node.data);
